...
pool.connect()
  .then(() => console.log('✅ PostgreSQL (Supabase) connecté !'))
...
const db = {
  run: (sql, params = []) => pool.query(convertSQL(sql, params), params).then(r => ({ lastID: r.rows[0]?.id || r.rowCount, changes: r.rowCount })),
  get: (sql, params = []) => pool.query(convertSQL(sql, params), params).then(r => r.rows[0] || null),
  all: (sql, params = []) => pool.query(convertSQL(sql, params), params).then(r => r.rows)
};
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(401).json({ message: "Identifiant introuvable" });
    ...
    res.json({ token, user: { nom: user.nom, prenoms: user.prenoms, role: user.role } });
  } catch (err) {
